...
const globalErrorHandler = (
  ...
  if (error instanceof Prisma.PrismaClientValidationError) {
    ...
    const fieldSpecificMessages: { [key: string]: string } = {
      promoCodes: `Invalid \`promoCodes\`. Expected a nested object or array of promo codes, not a string. Example: \`promoCodes: [{ id: 'some-id' }]\`.`,
    };
  ...
  // Prisma Known Request Errors
    ...
    if (error.code === "P2025") {
      ...
      const fieldMatch = lastLine.match(/Argument `(.+?)`:/);
      const fieldName = fieldMatch ? fieldMatch[1] : "unknown_field";
      const developerMessage = lastLine.replace(
        `Argument \`${fieldName}\`: `,
        ""
      );
      errorMessages.push({
